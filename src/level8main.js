// Level 8 Main Function

G.F.loadLevel8 = function () { 
    
    // assigns new AI function to G.B.level8.AI
    this.AI = G.F.mainAI;
    ...
    // adds holes to viewport with holeAI 
    G.makeGob('hole0', G.O.viewport)
     .setVar({x:235, y:5, w:30, h:490, AI:G.F.holeAI})
     .setState({horizontal: false})
     .addClass('hole')
     .turnOn();    
    G.makeGob('hole1', G.O.viewport)
     .setVar({x:5, y:250, w:490, h:30, AI:G.F.holeAI})
     .setState({horizontal: true})
     .addClass('hole')
     .turnOn();
     
    // adds bridges to viewport 
    G.makeGob('bridge0', G.O.viewport, 'IMG')
     .setVar({x:100, y:240, 
              ...
              cx: 0, cy: 0,
              AI: G.F.bridgeAI,
              nextSrc:'img/Vbridge.png'})
     .setState({horizontal: false, id: 0});   
     
    G.makeGob('bridge1', G.O.viewport, 'IMG')
     .setVar({x:225, y:365, 
              w:50, h:50,
              tw: 50, th: 50,
              cw: 50, ch: 50,
      ...
      .setState({horizontal: true, id: 1});
                
    G.makeGob('bridge2', G.O.viewport, 'IMG')
     .setVar({x:400, y:240, 
              w:50, h:50,
              tw: 50, th: 50,
              cw: 50, ch: 50,
              ...
              AI: G.F.bridgeAI,
              nextSrc:'img/Vbridge.png'})
     .setState({horizontal: false, id: 2});   
     
    // adds bridgeSwitches to viewport with bridgeSwitchAI
    G.makeGob('bridgeSwitch0', G.O.viewport, 'IMG')
     ...
     .setState({id: 0})
     .turnOn();
    G.makeGob('bridgeSwitch1', G.O.viewport, 'IMG')
     .setVar({x:185, y:40, 
              w:20, h:20, 
              tw: 20, th: 20,
              cw: 20, ch: 20,
     ...
     .setState({id: 1})
     .turnOn();
    G.makeGob('bridgeSwitch2', G.O.viewport, 'IMG')
     .setVar({x:150, y:420, 
              w:20, h:20, 
              tw: 20, th: 20,
              cw: 20, ch: 20,
    ...
    // add walls to viewport with wallAI
...
G.F.mainAI = function () 
{
        ...
        for (var i = 0; i < 2 ; i ++)
        {
            // starts AI for enemies
            G.O['enemy' + i].AI();
            // starts AI for holes
            G.O['hole' + i].AI();
            // starts AI for switches
            G.O['switch' + i].AI();
            // starts AI for gates
            G.O['gate' + i].AI();
            // starts AI for walls
        ...
        for (var i = 0; i < 3; i++)
        {
            // starts AI for bridge
            G.O['bridge' + i].AI();
            // starts AI for bridgeSwitch
            G.O['bridgeSwitch' + i].AI();
        }
...
// creates and loads level8 block which starts the level
$(document).ready(function() {
    G.makeBlock('level8', G.F.loadLevel8).loadBlock('level8');
});
